...
namespace Flocking
{
    ...
    [BurstCompile]
    public partial struct SystemSpawnFlockingGathering : ISystem
    {
        private EntityQuery _boidQuery;
        public void OnCreate(ref SystemState state)
        {
            _boidQuery = new EntityQueryBuilder(Allocator.Temp).WithAll<SharedComponentFlockingSettings, LocalTransform>().Build(ref state);
        }
        ...
        public void OnUpdate(ref SystemState state)
        {
            ...
            foreach (var (boidSchool, boidSchoolLocalToWorld, entity) in
                     SystemAPI.Query<RefRO<ComponentFlockingGathering>, RefRO<LocalToWorld>>()
                         .WithEntityAccess())
            {
                var boidEntities =
                ...
                var setBoidLocalToWorldJob = new SetFlockingLocalToWorld
                {
    ...
    [BurstCompile]
    struct SetFlockingLocalToWorld : IJobParallelFor
    {
